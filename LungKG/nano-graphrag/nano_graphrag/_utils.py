...
class TokenizerWrapper:
    ...
    def get_tokenizer(self):
        """Provide access to the underlying tokenizer object for special cases (e.g., decode_batch)."""
        self._lazy_load_tokenizer()
    ...
    # +++ Added +++: Add a batch decode method for efficiency and interface consistency
    def decode_batch(self, tokens_list: list[list[int]]) -> list[str]:
        self._lazy_load_tokenizer()
        # HuggingFace tokenizer has decode_batch, but tiktoken doesn't; we simulate with list comprehension
        if self.tokenizer_type == "tiktoken":
...
def truncate_list_by_token_size(
    ...
    for i, data in enumerate(list_data):
        tokens += len(tokenizer_wrapper.encode(key(data))) + 1  # Defensive: simulate list concatenation via newlines
